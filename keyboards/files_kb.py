...
async def found_files_kb(
    message, file_cache: FilesCache, target: str, page: int = 0, items_per_page: int = 5
):
    ...
    logger.debug(
        f"Target is '{target}' Found files {files} in all files {len(file_cache.cache)}"
    )
    # Расчет пагинации
    ...
    for _file in current_files:
        builder.button(
            ...
            callback_data=f"file:{_file.id}",
        )

    # Добавляем кнопки навигации
    if page > 0:
        builder.button(text="◀️ Назад", callback_data=f"page:{target}:{page - 1}")
    if page < total_pages - 1:
        builder.button(text="Вперед ▶️", callback_data=f"page:{target}:{page + 1}")
